...
class UsersStorageClass: UsersStorageProtocol {
    ...
    required init?(users: [UserInitialData], followers: [(GenericIdentifier<UserProtocol>, GenericIdentifier<UserProtocol>)], currentUserID: GenericIdentifier<UserProtocol>) {
        ...
        self.users = users.map {
            userInitialInfo in let user = User(id: userInitialInfo.id, username: userInitialInfo.username, fullName: userInitialInfo.fullName,avatarURL: userInitialInfo.avatarURL, currentUserFollowsThisUser: userInitialInfo.id == currentUserID ? false : followers.contains(where: {
