//
//  PostsStorageClass.swift
//  FirstCourseFinalTask
//
//  Created by Melkozavr on 23.04.2020.
...
class PostsStorageClass: PostsStorageProtocol {
    ...
    required init(posts: [PostInitialData], likes: [(GenericIdentifier<UserProtocol>, GenericIdentifier<PostProtocol>)], currentUserID: GenericIdentifier<UserProtocol>) {
        self.posts =  posts.map { postInfo in
            let post = Post(id: postInfo.id, author: postInfo.author, description: postInfo.description, imageURL: postInfo.imageURL, createdTime: postInfo.createdTime, currentUserLikesThisPost: likes.contains(where:{$0.0 == currentUserID && $0.1 == postInfo.id }), likedByCount: likes.filter({ $0.1 == postInfo.id }).count)
            return post
        }
